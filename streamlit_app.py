...
if st.button("🔁 استعادة جميع النتائج المحذوفة"):
    st.session_state.hidden_results.clear()
    st.rerun()
...
for i, res in enumerate(filtered_results, start=1):
    unique_key = f"{res['الملف']}_{i}"
    if unique_key in st.session_state.hidden_results:
        continue  # تخطي النتيجة المحذوفة
    ...
    if st.button(f"❌ حذف النتيجة {i}", key=f"delete_{unique_key}"):
        st.session_state.hidden_results.add(unique_key)
        st.rerun()
