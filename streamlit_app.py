import streamlit as st import os import re import io import base64 import docx2txt import fitz  # PyMuPDF

إعداد الصفحة

st.set_page_config(page_title="🔍 البحث في أحكام المحكمة العليا", layout="wide") st.title("🔍 أداة البحث في ملفات PDF و Word")

دالة لتوحيد النص (إزالة التشكيل والمدود)

def normalize_text(text): text = re.sub(r'[ًٌٍَُِّْـ]', '', text)  # Remove diacritics text = re.sub(r'[ـ]', '', text)  # Remove tatweel return text

واجهة المستخدم

uploaded_files = st.file_uploader("📂 قم برفع الملفات (Word أو PDF)", type=["pdf", "docx"], accept_multiple_files=True)

st.markdown("### 📝 وصف البحث القانوني") description = st.text_area("اكتب وصفًا تفصيليًا عن نوع الأحكام التي تبحث عنها", height=150)

keywords = st.text_input("🔑 الكلمات المفتاحية (افصل بينها بفاصلة)")

if st.button("ابدأ البحث"): if not uploaded_files: st.warning("يرجى رفع بعض الملفات.") st.stop()
