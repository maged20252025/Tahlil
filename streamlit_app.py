...
# اختيار طريقة البحث: كل الكلمات أو أي كلمة
    search_mode = st.radio("طريقة البحث:", ["كل الكلمات", "أي كلمة"], horizontal=True)

# إظهار خيار طريقة البحث بعد تفعيل الزر
if st.session_state.get("search_triggered"):
    search_mode = st.radio("طريقة البحث:", ["كل الكلمات", "أي كلمة"], horizontal=True, key="search_mode_key")
...
if uploaded_files and st.session_state.get("search_triggered"):
    ...
    for uploaded_file in files_to_search:
        ...
        for paragraph in text_blocks:
            ...
            match_all = all(re.search(rf"{re.escape(k)}", norm_paragraph, re.IGNORECASE) for k in keyword_list)
            match_any = any(re.search(rf"{re.escape(k)}", norm_paragraph, re.IGNORECASE) for k in keyword_list)

            if (search_mode == "كل الكلمات" and match_all) or (search_mode == "أي كلمة" and match_any):
                highlighted = paragraph
                for raw_kw in raw_keywords:
                    highlighted = highlight(highlighted, raw_kw)
                results.append({"الملف": file_name, "نص": highlighted})
                matched_files[file_name] = file_copy.getvalue()
