...
def find_exact_matches(sentence, keyword):
    return re.search(rf'\b{re.escape(keyword)}\b', sentence, re.IGNORECASE)

def highlight_keyword(sentence, keyword):
    return re.sub(rf'(\b{re.escape(keyword)}\b)', r"<mark style='background-color: #fff176'>\1</mark>", sentence, flags=re.IGNORECASE)
...
if uploaded_files and st.session_state.get("search_triggered"):
        ...
        for paragraph in text_blocks:
            ...
            for sentence in sentences:
                for keyword in raw_keywords:
                    if find_exact_matches(sentence, keyword):
                        highlighted = highlight_keyword(sentence.strip(), keyword)
    ...
    if results:
        ...
        filtered_results = [
            ...
            and (filtered_keyword == "الكل" or find_exact_matches(r["نص"], filtered_keyword))
        ]
