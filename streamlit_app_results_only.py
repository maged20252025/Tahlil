...
# 📁 زر لتنزيل أسماء الملفات بدون الامتدادات
if uploaded_files:
    file_names = [Path(f.name).stem for f in uploaded_files]
    joined_names = "\n".join(file_names)
    st.download_button("📄 تحميل أسماء الملفات بدون الامتداد", data=joined_names, file_name="file_names.txt", mime="text/plain")
...
if uploaded_files and st.session_state.get("search_triggered"):
    ...
    if results:
        ...
        # 📄 زر لتحميل أسماء الملفات التي ظهرت فيها نتائج فقط (بدون الامتداد)
        result_file_names = sorted(set(Path(r["الملف"]).stem for r in filtered_results))
        result_text = "\n".join(result_file_names)
        st.download_button("📄 تحميل أسماء ملفات النتائج فقط", data=result_text, file_name="نتائج_البحث.txt", mime="text/plain")
