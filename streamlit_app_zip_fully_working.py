...
# كلاس يحاكي UploadedFile لتمرير الملفات من داخل ZIP بشكل سليم
...
uploaded_files_1 = st.file_uploader("📂 Browse files 1", type=["docx", "pdf"], accept_multiple_files=True, key="upload_1")
uploaded_files_2 = st.file_uploader("📂 Browse files 2", type=["docx", "pdf"], accept_multiple_files=True, key="upload_2")
uploaded_files_3 = st.file_uploader("📂 Browse files 3", type=["docx", "pdf"], accept_multiple_files=True, key="upload_3")
uploaded_files_4 = st.file_uploader("📂 Browse files 4", type=["docx", "pdf"], accept_multiple_files=True, key="upload_4")
uploaded_files_5 = st.file_uploader("📂 Browse files 5", type=["docx", "pdf"], accept_multiple_files=True, key="upload_5")
...
# رفع ملف مضغوط ZIP يحتوي على ملفات DOCX/PDF
zip_file = st.file_uploader("📦 أو ارفع ملف مضغوط بصيغة ZIP يحتوي على DOCX/PDF", type=["zip"], key="zip_uploader")
        ...
        with zipfile.ZipFile(zip_file) as archive:
            for name in archive.namelist():
                if name.lower().endswith((".docx", ".pdf")):
                    ...
                    fake_file = FakeUploadedFile(content, name)
                    uploaded_files.append(fake_file)
                    st.write(f"📄 تم استخراج الملف من ZIP: {name}")
    except Exception as e:
        st.error(f"حدث خطأ أثناء فك ضغط الملف: {e}")
...
for i in range(1, 6):
    uploaded_group = globals().get(f"uploaded_files_{i}")
    if uploaded_group:
        uploaded_files.extend(uploaded_group)
...
if uploaded_files and st.session_state.get("search_triggered"):
    ...
    for uploaded_file in files_to_search:
        ...
        if file_ext == "docx":
            ...
            pdf = fitz.open(stream=io.BytesIO(file_bytes), filetype="pdf")  # PDF تعامل طبيعي
            for page in pdf:
                text = page.get_text()
